...
- A later docs-only successor CI `30028752748` exposed two first-attempt desktop Archive Gate visual flakes while both retries passed. Downloaded actual/expected/diff showed the preceding WebGL back buffer and the 5.2-second controls-hint timer state on a slow first render. The fixture now pins the intended hint and awaits two real animation frames after paused room placement instead of assuming a 120 ms wall-clock delay rendered the final scene. No baseline changed. Focused repeats passed 20/20 desktop and 10/10 mobile.
- Exact successor CI `30030864715` then passed all product and visual assertions but hit one Windows-runner `net::ERR_NO_BUFFER_SPACE` on the initial local Vite navigation; the retry passed and the suite ended 67 passed / 14 intentional skips / 1 infrastructure flaky. Navigation now retries only that exact socket-buffer error once; other navigation failures still fail immediately.
